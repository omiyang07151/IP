...
KISSY.add('gallery/uploader/1.5/theme',function (S, Node, Base) {
    ...
    S.extend(Theme, Base, /** @lends Theme.prototype*/{
        ...
        _appendFileDom:function (fileData) {
            ...
            var hFile;
            if (!$target.length) return false;
            var urlInputName = self.get('urlInputName');
            var sortInputName = self.get('sortInputName');
            var tplTmp = tpl.replace('{urlinput}',urlInputName);
            tplTmp = tplTmp.replace('{sortinput}',sortInputName);
            hFile = S.substitute(tplTmp, fileData);
...
KISSY.add('gallery/uploader/1.5/themes/imageUploader/index',function (S, Node, Theme) {
    ...
    S.extend(ImageUploader, Theme, /** @lends ImageUploader.prototype*/{
        ...
        _changeImageSrc:function(ev){
            ...
            var sUrl = result.name;
            var $imgUrl = $('._J_Pic_Url_'+id);
            $imgUrl.val(sUrl);

            var $imgSort = $('._J_Pic_Sort_'+id);

            if(result.sort) {
                var sSort = result.sort;
                $imgSort.val(sSort);
            } else {
                var self = this;
                var sort = self.get('sortIndex');
                $imgSort.val(sort);
                sort++;
                self.set('sortIndex',sort);
            }
        }
        ...
        '<div style="width:100%" class="_pic_sort">'+
        '<input type="hidden" class="_J_Pic_Url_{id}" name="{urlinput}[{id}]" value="">'+
        //omiyang cancel
            //'<input type="text" class="_J_Pic_Sort_{id}" name="{sortinput}[{id}]" value="" style="width: 28px;margin: 4px 40%;text-align: center;">'+
        '</div>'+
        '</li>'
        },
        /**
